...
class IgMdsrNmf:
    ...
    def model(self):
        ...
        print(model.summary())
        f = self.nme + "/" + self.nme + "_igmdsrnmf_best_model_" + str(self.dc) + "_" + str(self.r[2]) + "_" + str(self.factor[2]) + ".h5"
        ...
        print(o1,o2)
        fname1 = self.nme + "/" + self.nme + "_igmdsrnmf_b_" + str(self.m) + "_" + str(self.r[2]) + "_" + str(self.factor[2]) + ".txt"
        f1 = open(fname1, "w")
        np.savetxt(f1, latent_output, fmt = '%.17f', delimiter = ',')
        f1.close()
        fname2 = self.nme + "/" + self.nme + "_igmdsrnmf_w_" + str(self.r[2]) + "_" + str(self.dc) + "_" + str(self.factor[2]) + ".txt"
        f2 = open(fname2, "w")
        np.savetxt(f2, output_weights[0], fmt = '%.17f', delimiter = ',')
        f2.close()
        fname3 = self.nme + "/" + self.nme + "_igmdsrnmf_cost_array_" + str(self.dc) + "_" + str(self.r[2]) + "_" + str(self.factor[2]) + ".txt"
        f3 = open(fname3, "w")
...
def main(data_choice, factor):
    ...
    obj = IgMdsrNmf(nme, x, dc, factors, red_dim)
    obj.model()
